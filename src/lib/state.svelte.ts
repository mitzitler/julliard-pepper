...
class Location {
    ...
    openMoveRoom() {this.open = true}
    closeMoveRoom() {this.open = false}
...
class Tasks {
    ...
    incrementTask(taskId: string, tasksBySteps: TaskListSteps) {
        ...
        if (this.completedTasks.includes(taskId) || this.failedTasks.includes(taskId)) {
            ...
            // return [nextTaskStep, nextTaskStepTarget]
            done = true
            return done
            ...
            nextTaskStepTarget = tasksBySteps[taskId].steps[nextTaskStep].target_type
            // return [nextTaskStep, nextTaskStepTarget]
            done = false
            return done
...
class Conversations {
        ...
        responseDialogue: { 
            if (!nonStandardDialogueDict[guy]) {
                ...
                for (const taskId of Object.keys(nonStandardDialogueDict[guy]['X'])) {
                    if (activeTasks.includes(taskId) && succeed === true) { // for now its always succeed
                        ...
                        return [this.dialogueArray, taskId, dialogueType]
                    } else {
    ...
    incrementTaskDialogue(guy: string) {
